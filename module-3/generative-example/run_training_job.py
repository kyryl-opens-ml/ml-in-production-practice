...
custom_image = Image.from_registry(
    "ghcr.io/kyryl-opens-ml/generative-example:main"
).env(env)
...
def run_generative_example():
    ...
    upload_to_registry(
        model_name="modal_generative_example",
        model_path=Path("/tmp/phi-3-mini-lora-text2sql"),
    )
    load_from_registry(
        model_name="modal_generative_example:latest",
        model_path=Path("/tmp/loaded-model"),
    )
